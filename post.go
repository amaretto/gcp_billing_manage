...
// PostBillingWithSKU posts billing info to GSS
func PostBillingWithSKU(ctx context.Context, billingInfo []AmountWithSKU) error {
	credentialPath := os.Getenv("CREDENTIAL_PATH")
	b, err := ioutil.ReadFile(credentialPath)
	if err != nil {
		log.Fatalf("Unable to read client secret file: %v", err)
	...
	spreadsheetID := os.Getenv("GSS_ID")
	targetSheetName := "monthly_billing_sku"
	...
	for _, billing := range billingInfo {
		//values[i] = []interface{}{billing.Month, billing.Project, billing.Total}
		if prjskuMap[billing.Project+","+billing.Service+","+billing.SKU] == 0 {
			prjskuMap[billing.Project+","+billing.Service+","+billing.SKU] = prjskuIdx
			prjskuIdx++
		}
		if mnthMap[billing.Month] == 0 {
			mnthMap[billing.Month] = mnthIdx
	...
	// set header
	rcreq := HeaderRequest(sheetID, 1, 2, 1, int64(len(mnthMap)+4)) // padding + Project + Service + SKU
	...
	// make 2 dimention slice
	nums := make([][]interface{}, len(prjskuMap)+1)
	for i := 0; i < len(prjskuMap)+1; i++ {
		nums[i] = make([]interface{}, len(mnthMap)+3)
	}
	// x header
	nums[0][0] = "Project"
	nums[0][1] = "Service"
	nums[0][2] = "SKU"
	for prjsku, idx := range prjskuMap {
		psarray := strings.Split(prjsku, ",")
		nums[idx][0] = psarray[0] // Project
		nums[idx][1] = psarray[1] // Service
		nums[idx][2] = psarray[2] // SKU
	}
	// y header
	for mnth, idx := range mnthMap {
		nums[0][idx+2] = mnth
	...
	for _, billing := range billingInfo {
		nums[prjskuMap[billing.Project+","+billing.Service+","+billing.SKU]][mnthMap[billing.Month]+2] = billing.Total
	}
	...
	// set border
	br := BorderRequest(sheetID, 1, int64(len(prjskuMap))+2, 1, int64(len(mnthMap))+4) // padding + Project + Service + SKU
